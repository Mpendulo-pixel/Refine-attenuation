    ...
    if not kat.dry_run:
        for ant in kat.ants:  # Main antenna loop
            ...
            if band not in bandlist:
                ...
                user_logger.info(' ')
                user_logger.info('looping through for : '+str(ant.name))
        ...
        summary_notes.append(' Report summary notes : ')
        summary_notes.append('---------------------------------------')

        changed_antennas = []
        changed_antennas.append(' ')
        changed_antennas.append(
            ' List of changed antennas (ant, H-pol, V-pol): ')
        changed_antennas.append('---------------------------------------')
        ...
        send_email(
            'msibiya@sarao.ac.za,meerkat-attenuation@sarao.ac.za',
            summary,
